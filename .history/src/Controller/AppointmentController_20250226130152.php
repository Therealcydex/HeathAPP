...
{
    $page = max(1, $request->query->getInt('page', 1)); // Get current page, default is 1
    $limit = 4; // Number of appointments per page
    $searchTerm = $request->query->get('search', ''); // Get search term
    ...
    if ($request->isXmlHttpRequest()) { // If AJAX request, return JSON
        return $this->json([
            'appointments' => array_map(function ($appointment) {
                return [
                    ...
                    'hasPrescriptions' => !$appointment->getPrescriptions()->isEmpty(),
                ];
            }, $appointments),
        ]);
    }
    ...
    return $this->render('appointment/view_appointments.html.twig', [
        ...
        'currentPage' => $page,
        'totalPages' => ceil($appointmentRepository->count([]) / $limit),
    ]);
}
