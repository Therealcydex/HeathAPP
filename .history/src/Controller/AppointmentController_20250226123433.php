...
public function book(Request $request): Response
{
    ...
    if ($form->isSubmitted() && $form->isValid()) {
        ...
        // Redirect to add appointment to Google Calendar
...
{
    $query = $request->query->get('search'); // Get the search query
    $page = max(1, $request->query->getInt('page', 1)); // Get current page
    $limit = 4; // Number of appointments per page
    $appointmentRepository = $this->entityManager->getRepository(Appointment::class);

    if ($query) {
        // If searching, find all appointments matching client name (ignoring pagination)
        $appointments = $appointmentRepository->createQueryBuilder('a')
            ->where('a.clientName LIKE :search')
            ->setParameter('search', "%{$query}%")
            ->orderBy('a.appointmentDate', 'ASC')
            ->getQuery()
            ->getResult();

        // Bypass pagination by setting totalPages to 1
        return $this->render('appointment/view_appointments.html.twig', [
            'appointments' => $appointments,
            'currentPage' => 1,
            'totalPages' => 1, // No pagination during search
            'searchQuery' => $query // Send back the search term to keep input field value
        ]);
    }

    // Default behavior: paginated appointments
    ...
    return $this->render('appointment/view_appointments.html.twig', [
        ...
        'searchQuery' => null // No search, keep input empty
    ]);
}
